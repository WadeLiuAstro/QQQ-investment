...
- 消息面展示卡片（独立卡片，纯展示）：用户拍板的需求钉死项——混合数据源（现有宏观日历打底 + CNBC RSS 增强，失败降级纯日历）；英文标题原样 + 中文来源名（CNBC 宏观/CNBC 头条）+ 原文链接；只列头条不做利多/利空判定；随日频刷新一天一次；最近 3 天最多 12 条、预期事件取最近 3 个；样式为用户选定的样本 C（预期事件卡 + 头条混排）；**永不耦合五档决策/仓位/倍率/闸门**；未来可与大涨/大跌归因联动，联动方式另行讨论。卡片位置在 monitoring-section 之后；旧快照无 `news` 字段时前端隐藏。
