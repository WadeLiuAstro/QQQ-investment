...
每位 agent 开始工作前按顺序阅读 `docs/AGENTS.md`、`docs/CONTEXT.md`、`docs/MEMORY.md`。这三份文件位于仓库 `docs/` 目录，用于替代对完整聊天历史的依赖：本文件说明**如何工作**，`docs/CONTEXT.md` 说明**系统现在是什么**，`docs/MEMORY.md` 记录**为何做过关键选择**。
...
4. 完成后把仍有效的架构决定或运行问题更新到 `docs/MEMORY.md`；不要把逐次对话、临时行情或提交日志塞进记忆文件。
...
3. 汇总实际改动、测试证据与残余风险。
4. 确认全部验收条件满足后，同步三份文档（roadmap / docs/CONTEXT.md / docs/MEMORY.md）并提交。
...
2. `docs/CONTEXT.md`：更新数据流、界面约定与文件地图等技术上下文（如 payload 新增字段、新增服务模块）。
3. `docs/MEMORY.md`：记录经确认的决策变更、用户偏好与已复现问题；只写长期事实，不写过程流水。
...
- `docs/AGENTS.md`：工作规则、验证命令、目录职责。仅在协作流程变化时更新。
- `docs/CONTEXT.md`：当前架构、数据流、规则接口与部署事实。系统结构变化时更新。
- `docs/MEMORY.md`：经用户确认的长期偏好、风险边界、已复现的数据源问题。不要记录临时行情、个人信息或冗长过程。