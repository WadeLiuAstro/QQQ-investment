...
| 角色 | 模型 | 职责 |
|---|---|---|
| 主线程 | Qwen3.8 | 需求分析、架构设计、任务拆分、风险判断、代码审查、最终验收 |
...
子代理在主 worktree 内按任务逐步 commit，不另开独立分支。每个任务派发时必须包含：

1. **目标**：一句话描述交付物。
2. **基础 commit**：当前 HEAD 哈希（回滚基准）。
3. **文件所有权**：本任务允许修改的文件清单，不得越界。
4. **验收标准**：可执行的测试命令或可观察的行为断言。
5. **约束**：不得自行扩大范围、不得创建或合并 PR、不得修改任务范围外的文件。

子代理执行流程：先写失败测试 → 最小实现 → 测试通过 → commit（含 conventional commits 前缀）。

完成后报告：commit 哈希、修改文件列表、测试结果、残余风险。

### 主线程验收流程

主线程收到子代理完成报告后，**不得仅凭文字报告判定完成**，必须执行以下核验：

1. 检查 `git status --short` 和完整 `git diff`，确认无越界文件修改。
2. 重新运行 `pytest`（至少覆盖涉及模块），确认测试通过。
3. 如涉及前端变更，做浏览器端到端核验。
4. 发现问题时，派发修正任务给新的子代理实例（附带具体问题描述和期望行为）。
5. 验收通过后方可进入下一任务。
